...
- 本轮不处理混合意图。没有 open pending interaction 时，`CLARIFICATION_ANSWER` 只能进入
  无目标保护提示，不能被局部规则改写为新任务；混合意图需下一轮作为独立路由能力设计。
